...
def info(update: Update, context: CallbackContext):
    ...
    if disaster_level_present:
        text += ' [<a href="https://t.me/Karunada_kings_and_queens">?</a>]'.format(
            bot.username
        )
...
def set_about_me(update: Update, context: CallbackContext):
    ...
    if user_id in [777000, 5272015055]:
    ...
    if message.reply_to_message:
        ...
        if repl_user_id in [bot.id, 777000, 5272015055] and (user_id in DEV_USERS):
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id in [777000, 5272015055] and sender_id not in DEV_USERS:
